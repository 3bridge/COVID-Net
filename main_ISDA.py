...
def main():
    ...
    test_acc_file = 'isdaruns/acont6-1/test' + str(args.cosine) + str(args.bna) + str(args.bnd) + '.txt'
    train_acc_file = 'isdaruns/acont6-1/train' + str(args.cosine) + str(args.bna) + str(args.bnd) + '.txt'
    ...
    model, fc, optimizer, training_generator, test_generator = initialize(args)

    # fc = Full_layer(model.module.feature_num, args.classes).cuda()

    isda_criterion = ISDALoss(model.feature_num, args.classes).cuda()
    ...
    for epoch in range(1, args.nEpochs + 1):
        ...
        train_metrics = train(args, model, fc, training_generator, optimizer, epoch, isda_criterion)

        torch.cuda.empty_cache()

        test_metrics, confusion_matrix, ucsd_correct_total, sars_correct_total, ucsd_test_total, sars_test_total \
            = validation(args, model, fc, test_generator, epoch, mode='test')

        best_pred_loss = util.save_model(model, fc, optimizer, args, test_metrics, epoch, best_pred_loss, confusion_matrix)
...
def get_arguments():
    ...
    parser.add_argument('--save', type=str, default='isdasaved/acont6-1',
                        help='path to checkpoint ')
    parser.add_argument('--cosine', default=True,
                        help='learning rate adjust scheme ')
    parser.add_argument('--bnd', default=True,
    ...
    parser.add_argument('--lambda_0', default=2.5, type=float,
