...
class CovidNet(nn.Module):
    def __init__(self, bnd=False, bna=False, label_smoothing=False,
        ...
        if bnd:
            ...
            # bn for ucsd data
            self.add_module('gn0', nn.Sequential(GroupNorm(num_features=64), nn.ReLU()))
            self.add_module('gn1', nn.Sequential(GroupNorm(num_features=256), nn.ReLU()))
            self.add_module('gn2', nn.Sequential(GroupNorm(num_features=512), nn.ReLU()))
            self.add_module('gn3', nn.Sequential(GroupNorm(num_features=1024), nn.ReLU()))
            self.add_module('gn4', nn.Sequential(GroupNorm(num_features=2048), nn.ReLU()))
            # bn for new data
            self.add_module('gn0new', nn.Sequential(GroupNorm(num_features=64), nn.ReLU()))
            self.add_module('gn1new', nn.Sequential(GroupNorm(num_features=256), nn.ReLU()))
            self.add_module('gn2new', nn.Sequential(GroupNorm(num_features=512), nn.ReLU()))
            self.add_module('gn3new', nn.Sequential(GroupNorm(num_features=1024), nn.ReLU()))
            self.add_module('gn4new', nn.Sequential(GroupNorm(num_features=2048), nn.ReLU()))
        else:
            self.add_module('conv1', nn.Conv2d(in_channels=3, out_channels=64, kernel_size=7, stride=2, padding=3))
            self.add_module('conv1_1x1', nn.Conv2d(in_channels=64, out_channels=256, kernel_size=1))
            self.add_module('conv2_1x1', nn.Conv2d(in_channels=256, out_channels=512, kernel_size=1))
    ...
    def forward(self, x, site):
        ...
        if self.bnd:
            if site == 'ucsd':
                conv1 = self.gn0(conv1)
            else:
                conv1 = self.gn0new(conv1)
        ...
        if self.bnd:
            if site == 'ucsd':
                out_conv1_1x1 = self.gn1(out_conv1_1x1)
            else:
                out_conv1_1x1 = self.gn1new(out_conv1_1x1)
        ...
        if self.bnd:
            if site == 'ucsd':
                conv2 = self.gn2(conv2)
            else:
                conv2 = self.gn2new(conv2)
        ...
        if self.bnd:
            if site == 'ucsd':
                conv3 = self.gn3(conv3)
            else:
                conv3 = self.gn3new(conv3)
        ...
        if self.bnd:
            if site == 'ucsd':
                conv4 = self.gn4(conv4)
            else:
                conv4 = self.gn4new(conv4)
