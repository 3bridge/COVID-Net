...
class CovidNet(nn.Module):
    def __init__(self, bnd=False, bna=False, label_smoothing=False,
        ...
        if bnd:
            ...
            # bn for ucsd data
            self.add_module('bn0', nn.Sequential(FRNLayer(num_features=64),TLU(num_features=64)))
            self.add_module('bn1', nn.Sequential(FRNLayer(num_features=256),TLU(num_features=256)))
            self.add_module('bn2', nn.Sequential(FRNLayer(num_features=512),TLU(num_features=512)))
            self.add_module('bn3', nn.Sequential(FRNLayer(num_features=1024),TLU(num_features=1024)))
            self.add_module('bn4', nn.Sequential(FRNLayer(num_features=2048),TLU(num_features=2048)))
            # bn for new data
            self.add_module('bn0new', nn.Sequential(FRNLayer(num_features=64),TLU(num_features=64)))
            self.add_module('bn1new', nn.Sequential(FRNLayer(num_features=256),TLU(num_features=256)))
            self.add_module('bn2new', nn.Sequential(FRNLayer(num_features=512),TLU(num_features=512)))
            self.add_module('bn3new', nn.Sequential(FRNLayer(num_features=1024),TLU(num_features=1024)))
            self.add_module('bn4new', nn.Sequential(FRNLayer(num_features=2048),TLU(num_features=2048)))
        else:
            self.add_module('conv1', nn.Conv2d(in_channels=3, out_channels=64, kernel_size=7, stride=2, padding=3))
            self.add_module('conv1_1x1', nn.Conv2d(in_channels=64, out_channels=256, kernel_size=1))
            self.add_module('conv2_1x1', nn.Conv2d(in_channels=256, out_channels=512, kernel_size=1))
